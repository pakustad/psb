...
class Model_data extends CI_Model
{
  ...
  function statistik($thn = '', $aksi = '')
  {
    ...
    $sql = $this->db->query("
    select
      ifnull((SELECT count(id_siswa) FROM (tbl_siswa)WHERE((Month(tgl_siswa)=8) $status AND (YEAR(tgl_siswa)='$now'))),0) AS `Agustus`,
      ifnull((SELECT count(id_siswa) FROM (tbl_siswa)WHERE((Month(tgl_siswa)=9) $status AND (YEAR(tgl_siswa)='$now'))),0) AS `September`,
      ifnull((SELECT count(id_siswa) FROM (tbl_siswa)WHERE((Month(tgl_siswa)=10) $status AND (YEAR(tgl_siswa)='$now'))),0) AS `Oktober`,
      ifnull((SELECT count(id_siswa) FROM (tbl_siswa)WHERE((Month(tgl_siswa)=11) $status AND (YEAR(tgl_siswa)='$now'))),0) AS `Nopember`,
      ifnull((SELECT count(id_siswa) FROM (tbl_siswa)WHERE((Month(tgl_siswa)=12) $status AND (YEAR(tgl_siswa)='$now'))),0) AS `Desember`,
